...
public class Radar extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand UsedHand){
        ...
        if (!level.isClientSide){
            ...
            // 获取实体列表，用List<Player>泛型集合类型存储玩家，防止存储其他实体
            // searchArea写进去防止搜索到区域外
            // p -> p != player 防止把自己放进集合里，同时也应该排除旁观者模式的玩家 (!p.isSpectator())
            ...
            if (nearestTarget != null){
                ...
                // --- 2. 给被发现的人发送警告消息（这是你要加的功能） ---
                // true 参数表示显示在 Action Bar (物品栏上方)
                // 使用红色加粗字体，更加醒目
